...
// as JSON. An [HTML] output writes content that was already rendered by the
// endpoint. Transport fields must also use json:"-" so metadata cannot leak
// into JSON representations. Input fields may use validate:"required,name" to apply
...
func endpointHandler[In, Out any](
	...
	return func(w http.ResponseWriter, request *http.Request) {
		...
		if err := writeEndpointOutput(w, route.status, output, outputMetadata); err != nil {
			writeError(logger, w, request, route, err)
		}
	}
}
